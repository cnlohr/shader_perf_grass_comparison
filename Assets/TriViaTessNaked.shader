    ...
    SubShader
    {
        ...
        Pass
        {
			...
			vtx hull( InputPatch<vtx, 4> IN, uint uCPID : SV_OutputControlPointID )
			{
				...
				o.batchID = uint4( uCPID, 0, 0, 0 );
				return o;
			}
	 
			[domain("quad")]
			vtx dom( tessFactors HSConstantData, const OutputPatch<vtx, 4> IN, float2 bary : SV_DomainLocation, uint sourceID : SV_PrimitiveID )
			{
				vtx o = (vtx)0;
				o.coloro = float4( bary.x, bary.y, sourceID/10.0, 1.0 );
				float3 lerped = float3( bary.x, bary.y, sin(bary.x*6.28) + sin(bary.y*6.28 ) ); 
				lerped.y += sourceID;
				o.vertex = UnityObjectToClipPos( float4( lerped.xyz, 1.0 ) );
/*
				float3 rootxyz = IN[0].vertex.xyz;
				const float bladethick = 4.0;
				uint instanceID = bary.y;
				uint batchID = bary.x;
				rootxyz.z += instanceID * 6.4;
				...
				uint height = uint( batchID) % 4;
				float bladesignthick = sign( batchID > 4.0 )?-bladethick:bladethick;
