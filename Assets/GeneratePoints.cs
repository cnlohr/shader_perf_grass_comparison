...
public class GeneratePoints : MonoBehaviour
{
	...
	[MenuItem("MyMenu/GenerateTriALot")]
    static void GenerateTriALot()
    {
		const int ptx = 2048;
		const int pty = 2048;
	...
	[MenuItem("MyMenu/GenerateTriFew")]
    static void GenerateTriFew()
    {
		const int ptx = 1;
		const int pty = 2048;
		const int idstride = 18;
		...
		mesh.vertices = verts;
		mesh.SetIndices(indices, MeshTopology.Triangles, 0, false, 0);
		AssetDatabase.CreateAsset(mesh, "Assets/trifew.asset");
    }
